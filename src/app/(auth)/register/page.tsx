...
// Turnstile would be imported here in a real impl
...
export default function RegisterPage() {
  ...
  const { login } = useAuth(); // using login mock after successful register
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // After success, login automatically
    await login(username);
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-4 py-12">
      <Link href="/" className="absolute top-4 left-4 md:top-8 md:left-8 flex items-center text-sm font-medium text-slate-500 hover:text-slate-900 transition-colors">
      ...
      <Card className="w-full max-w-md shadow-xl border-slate-200/60 dark:border-slate-800">
        <CardHeader className="space-y-2 pb-6 text-center">
          <div className="flex justify-center mb-4">
            <span className="font-bold text-3xl tracking-tight text-primary">JokiSystem.</span>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                required
              />
              <p className="text-xs text-slate-500">Hanya huruf kecil, angka, dan dash (-). Contoh: izumi-store</p>
            </div>
            ...
            <div className="w-full flex justify-center py-2">
               <div className="h-[65px] w-[300px] bg-slate-100 dark:bg-slate-800 rounded flex items-center justify-center border border-slate-200 dark:border-slate-700">
                  <span className="text-xs text-slate-400">Cloudflare Turnstile Verification</span>
               </div>
            </div>
